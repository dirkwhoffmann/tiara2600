...
#define FLOAT_TIARA_ERROR_MARGIN 0.000001
...
- (void)insertItemAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (fabs(_itemWidth) < FLOAT_TIARA_ERROR_MARGIN)
    {
        [self updateItemWidth];
    }
...
- (void)step
{
    ...
    else if (_decelerating)
    {
        ...
        [self didScroll];
        if (fabs(time - _scrollDuration) < FLOAT_TIARA_ERROR_MARGIN)
        {
            _decelerating = NO;
            [self pushAnimationState:YES];
            [_delegate carouselDidEndDecelerating:self];
            [self popAnimationState];
            if ((_scrollToItemBoundary || fabs(_scrollOffset - [self clampedOffset:_scrollOffset]) > FLOAT_TIARA_ERROR_MARGIN) && !_autoscroll)
            {
                if (fabs(_scrollOffset - self.currentItemIndex) < FLOAT_TIARA_ERROR_MARGIN)
                {
    ...
    else if (fabs(_toggle) > FLOAT_TIARA_ERROR_MARGIN)
    {
...
- (void)didScroll
{
    ...
    if (fabs(_scrollOffset - _previousScrollOffset) > FLOAT_TIARA_ERROR_MARGIN)
    {
        [self pushAnimationState:YES];
